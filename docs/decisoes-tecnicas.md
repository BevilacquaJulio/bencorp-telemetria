...
risco de consistência distribuída sem benefício proporcional para um case de
cinco dias.
...
## 2. PostgreSQL e Prisma com garantias também no banco

**Decisão:** Prisma para acesso tipado e migrations, complementado por índices
parciais, triggers e SQL transacional onde o ORM não expressa a regra.
...
múltiplas instâncias e chamadas que não passam pelo frontend.

**Trade-off:** parte das garantias usa recursos específicos do PostgreSQL. A
portabilidade para outro banco exigiria migrations equivalentes, mas a solução
ganha correção sob concorrência.
...
foco das regras clínicas e de segurança do case.
...
**Decisão:** o serviço `migrate` aplica migrations e executa o seed antes da API
subir.

**Motivo:** migrations não pertencem ao build da imagem e não devem disputar
entre réplicas da aplicação. O seed é idempotente e prepara a demonstração.

**Trade-off:** em produção, seed e migration poderiam ser comandos separados e
executados por pipeline. Para o case, a automação torna a execução reproduzível
com um único `docker compose up`.

## 14. Docker local autocontido

**Decisão:** o Compose inclui PostgreSQL, LiveKit, migração, API e frontend.

**Motivo:** o avaliador consegue executar o recorte completo sem serviços pagos
ou infraestrutura externa.

**Trade-off:** a topologia usa HTTP/WebSocket local e não inclui TLS, TURN
público, alta disponibilidade ou observabilidade de produção. Esses pontos são
detalhados em [limitações](./limitacoes.md).
