...
| Índice único parcial no PostgreSQL | Nada. O banco recusa a segunda linha |
...
| 2 | Um profissional nunca tem dois atendimentos `EM_ANDAMENTO` | `CREATE UNIQUE INDEX ... ON ("profissionalId") WHERE status = 'EM_ANDAMENTO'` | Mesmo profissional inicia dois atendimentos → `409` |
