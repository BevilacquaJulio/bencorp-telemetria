...
const testPort = process.env.PAD_TEST_PORT ?? '5433';
...
const testEnvironment = {
  ...
  DATABASE_URL: `postgresql://pad:pad@127.0.0.1:${testPort}/pad_test?schema=public`,
...
try {
  ...
  execute('Criando PostgreSQL temporário pad_test', 'docker', [
    ...composeArguments,
    'up',
    '--detach',
    '--wait',
  ...
  if (dockerAvailable) {
    console.log('\n[E2E] Removendo PostgreSQL temporário');
...
if (failure) {
  ...
  console.log(
    '\n[E2E] Concluído: testes aprovados e pad_test temporário removido.',
  );
}
