...
// assim que o `docker compose up` termina.
//
// Roda automaticamente no boot do container da API (ver docker-compose) e
// também pode ser chamado à mão com `npx prisma db seed`. Os dois caminhos
// precisam ser seguros de repetir: por isso todo upsert usa `update: {}` —
// cria o que falta e nunca sobrescreve o que já existe. Sem isso, reiniciar
// o container depois de alguém ter mexido nos dados (ex.: iniciado um dos
// atendimentos AGUARDANDO durante um teste manual) apagaria esse progresso.
...
// Senha única pros 5 usuários de teste, documentada em texto no README do
// projeto — só o hash vai para o banco. Não é segredo real: é seed de dev.
...
async function main(): Promise<void> {
  ...
  console.log('\nSeed concluído.\n');
  console.log('Credenciais de teste (mesma senha pros 5 usuários):');
